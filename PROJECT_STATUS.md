...
| Apify | 🔌 conexión disponible, sin uso | Cliente de la API (`radar/fuentes/apify.py`: probar token, ejecutar Actor con tope de coste y leer el dataset) y token + tope mensual en **Ajustes**. No está conectado al agente. Es un servicio aparte de Claude (cuenta y facturación propias) y el cliente rechaza Actores o entradas que apunten a LinkedIn, redes sociales o Google Maps (sus condiciones prohíben el acceso automatizado; Apify traslada la responsabilidad a quien lo lanza). Probado con un token falso: Apify responde 401 y no se filtra el token. |
