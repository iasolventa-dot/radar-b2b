...
| Apify | ✅ conectada (opcional por búsqueda) | Cliente genérico de la API (`radar/fuentes/apify.py`: probar token, ejecutar cualquier Actor con tope de coste, leer dataset) y token + tope mensual en **Ajustes**. Herramienta del agente `enriquecer_con_apify` (rastrea webs propias de empresa con `apify/website-content-crawler` y pasa el texto por las mismas reglas que cualquier web). Como Google Places, solo se ofrece al planificador si se marca su casilla en «Nueva búsqueda» y hay token (`busquedas.opciones`). Servicio de terceros con cuenta y facturación propias. Probado con un token falso: Apify responde 401 y no se filtra el token; la ejecución real de un Actor no se ha probado. |
