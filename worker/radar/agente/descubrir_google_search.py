...
from radar.agente.herramientas import _url_no_apta_para_enriquecer
...
async def descubrir_google_search(
    ...
    *,
    ...
    entrada = {"queries": "\n".join(consultas), "maxPagesPerQuery": max(1, max_paginas_por_consulta), "countryCode": "es", "languageCode": "es"}
    res = await ejecutar_actor(cliente_http, token, ACTOR_GOOGLE_SEARCH, entrada, max_coste_usd=tope, timeout_s=120)
    ...
    for item in res.items:
        for organico in item.get("organicResults") or []:
            ...
            if dominio in _DOMINIOS_FACEBOOK:
                if url not in candidatos_facebook and len(candidatos_facebook) < MAX_CANDIDATOS_DEVUELTOS:
                    candidatos_facebook.append(url)
                continue
            if dominio in _DOMINIOS_LINKEDIN:
                if url not in candidatos_linkedin and len(candidatos_linkedin) < MAX_CANDIDATOS_DEVUELTOS:
